 ...
 *
 *  ВЕБ (волна 6, 2026-08-02): раздел целиком рисуется и во вкладке браузера —
 *  ни один ряд не помечен умением площадки, и это не недосмотр. Почта и
 *  пароль меняются серверными запросами (api.changeEmail/changePassword), а
 *  единственная «настольная» деталь — открыть ссылку подтверждения — умеет
 *  ОБЕ площадки через розетку (system.openExternal: программа отдаёт ссылку
 *  системному браузеру, страница открывает новую вкладку). Устройства,
 *  анонимная статистика, документ о данных, выгрузка и удаление аккаунта —
 *  тоже сервер. Прятать здесь нечего. */
