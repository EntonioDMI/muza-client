...
describe("ширина колонок админки", () => {
   ...
   *
   *  Пересняты 11.08: колонка даты выросла 104→128px. Прежние 104 были УЖЕ
   *  своего содержимого — «21.07, 12:34» в Golos 15px занимает 93.3px, а под
   *  содержимое оставалось 80px, и дата обрезалась на любом окне (жалоба
   *  владельца). Публикации при этом упёрлись бы в потолок, поэтому колонка
   *  ника отдала им 8px (104→96) — отсюда и «Темы» 440→432. */
  it("две бывшие нарушительницы влезли: 910→652 и 700→616", async () => {
    render(<AdminView api={makeApi()} />);
    await waitFor(() => expect(screen.getByText("Listeners")).toBeTruthy());
    ...
    expect(tableMinWidths()).toEqual([
      ...
      { name: "Public playlists", minWidth: 652 },
      { name: "Themes on the market", minWidth: 432 },
    ]);

    await openTab("Users", /Total 1/);
    expect(tableMinWidths()).toEqual([{ name: "Users", minWidth: 616 }]); // было 700px
  });
});
