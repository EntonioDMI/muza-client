              ...
              <Shelf title={s.title} {...shelfL10n(t)}>
                {s.tracks.map((tr) => (
                  ...
                  >
                    ...
                    />
                    {/* ⚠️ ПОДПИСИ «ПОЧЕМУ ЭТО ЗДЕСЬ» ПОД ПЛИТКОЙ БОЛЬШЕ НЕТ
                        (снята 13.08 по прямому указанию владельца: «убери под
                        плашками треков пояснения почему он попал сюда»).

                        Она стояла тут один день и была заведена на замерах
                        чужой пользы (Berkeley: понятная причина поднимает
                        среднюю оценку трека с 2.79 до 3.51). На нашей ширине
                        плитки замеры не сбылись: строка не помещалась и
                        обрывалась многоточием на середине — «Похоже на
                        „ptasinski“ по La…». Объяснение, которое нельзя
                        дочитать, объяснением не является, а место под каждой
                        плиткой занимает.

                        ⚠️ Причина по-прежнему ПРИЕЗЖАЕТ с сервера и в
                        контракте остаётся (Track.reason, серверный
                        recs/explain.ts): по ней мы решаем, какой слой
                        рекомендаций чинить, и терять её нельзя. Убран показ,
                        а не данные. */}
                  </div>
                ))}
              </Shelf>
