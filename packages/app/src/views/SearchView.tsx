 ...
 *  `results ?? []` создавал бы новый массив каждым рендером, а он уходит в
 *  зависимости эффекта onResultsChange: площадка ставила бы себе состояние с
 *  новым массивом, оно вызывало бы рендер — и так по кругу. */
 ...
 *
 *  ── ВОЛНА «ЭКРАНЫ» ВЕБ-ПАРИТЕТА (2026-08-02) ──
 *  Экран переехал из apps/desktop в @muza/app: теперь ОДИН и тот же поиск
 *  рисуют оба клиента, и урезанной веб-копии (своя страница + GroupedTrackList)
 *  больше нет. Приложенческое в экране осталось ровно двумя необязательными
 *  местами, оба по правилу розетки «нет умения → нет поведения»:
 *  - вынос файла на рабочий стол (Alt+перетаскивание) — через useAltFileDrag();
 *    в браузере хук вернёт false, и отрабатывает обычный перенос в плейлист;
 *  - прогрев строк (`warmRow`) — проп, который передаёт только приложение.
 *  Панель массовых действий тоже собирается по умениям площадки: пункт есть,
 *  только если действие у площадки существует (см. selectionActions ниже). */
export function SearchView({
  ...
  /** Открыть плейлист SoundCloud из выдачи (2026-07-20). Получает КАРТОЧКУ
   *  целиком, а не один id: у приложения для неё есть своя страница по id, а
   *  браузеру нужна ссылка на первоисточник (permalinkUrl). */
  onOpenScPlaylist: (playlist: PublicPlaylist) => void;
  ...
  const flatTracks = searchGrouping ? groupedFlat : (results ?? NO_TRACKS);
  const multi = useMultiSelect(flatTracks.map((tr) => tr.id));
  const selectedTracks = () => flatTracks.filter((tr) => multi.has(tr.id));
  // Набор умений объявлен НЕПОЛНЫМ (MenuAbilities, а не MenuContext): экран
  // общий, и в браузере половины действий очереди попросту нет.
  const { openMenu, menuCtxRef } = useContextMenu<MenuAbilities>();
  const altFileDrag = useAltFileDrag();

  // Площадке бывает нужен плоский список показанного (веб собирает по нему
  // меню трека). Приложение проп не передаёт — эффект вырождается в пустой.
  useEffect(() => {
    onResultsChange?.(flatTracks);
  }, [flatTracks, onResultsChange]);
  ...
  const renderRow = (tr: Track, index?: number, versions?: VersionsSlot) => (
    <div
      ...
      onDragStart={(e) => {
        ...
        // В браузере вынести файл нечем — хук вернёт false, и жест гаснет.
  ...
  /** Действия панели выделения. Пункт есть, ТОЛЬКО если площадка умеет
   *  действие: у приложения есть все пять (его панель не изменилась ни на
   *  пиксель), у браузера нет ни очереди, ни хранения на устройстве — там
   *  останется два. Правило то же, что у розетки платформы: нет умения — нет
   *  пункта, а не серый пункт, который ничего не делает. */
