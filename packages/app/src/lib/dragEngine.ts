...
/** В КАКОЙ ЧУЖОЙ ЯЧЕЙКЕ СЕЙЧАС ТОЧКА ЗАХВАТА. −1 — ни в какой (зазор между
 *  ячейками или своя собственная). Прямоугольник сжат на гистерезис: касание
 *  самой кромки дрожащей рукой попаданием не считается.
 *
 *  ⚠️ ЭТО ОТВЕТ НА ВОПРОС «ГДЕ», А НЕ «КУДА». Функция НЕ решает, надо ли
 *  переставлять: у неё нет памяти о прошлом кадре, и подряд идущие вызовы будут
 *  отвечать одинаково, пока курсор стоит внутри одной ячейки. Решение «пора
 *  менять местами» принимает useLocalReorder, сравнивая ответ с предыдущим, —
 *  и именно эта разница и есть ВХОД в ячейку.
 *
 *  ПОЧЕМУ ЭТО ВАЖНО (разбор 2026-08-05). Дока обещала правило «точка захвата
 *  ВОШЛА в чужую ячейку», а код проверял «НАХОДИТСЯ в чужой ячейке» — и это
 *  разные вещи. Утверждение «после обмена точка захвата оказывается в своей
 *  ячейке, повторного срабатывания нет» верно только когда тащимый НЕ МЕНЬШЕ
 *  вытесняемого. Арифметика для ленты статистики: тащим «Лайки» (высота ≈150)
 *  через «Топ треков» (≈420, во всю ширину, всегда один в строке). Курсор
 *  где-то внутри диапазона [y₀, y₀+420]. После обмена «Топ треков» начинается
 *  на y₀+150+зазор, то есть накрывает курсор снова при y > y₀+178 — а это
 *  242 из 420 пикселей, 58% площади большого блока. Условие срабатывало опять,
 *  в обратную сторону, и так по кругу. Владелец: «большой блок буквально
 *  прыгает вверх-вниз при микродвижении мыши». */
export function gridHitIndex(rects: readonly Box[], from: number, x: number, y: number): number {
...
/** Splice-индекс вставки в СЕТКЕ/ленте по точке захвата. Обёртка над
 *  gridHitIndex: «никуда не вошёл» превращается в «остаёмся на месте», потому
 *  что для жеста ничего не делать безопаснее, чем угадывать.
 *
 *  ⚠️ Живой потребитель — LibraryView (плитки одного размера, вне режима
 *  правки). Там «своя ячейка после обмена» выполняется честно: плитки
 *  одинаковые, вытеснять больше себя нечем, и осцилляции не возникает. Режим
 *  правки вида ходит через gridHitIndex + память о входе в useLocalReorder.
 *
 *  Арифметика splice сходится без поправок: ячейка i левее тащимого —
 *  splice(i); правее — после изъятия всё съехало на единицу, и «место ячейки
 *  i» — тоже splice(i). */
export function gridInsertionIndex(rects: readonly Box[], from: number, x: number, y: number): number {
  const i = gridHitIndex(rects, from, x, y);
