 ...
 *
 *  ⚠️ ПЕРЕЕХАЛО из apps/desktop/src/lib/shareCard.ts (волна экранов
 *  веб-паритета, 2026-08-02) вместе со своим единственным потребителем —
 *  диалогом «Поделиться». На старом месте пенёк-ре-экспорт: App.tsx берёт
 *  оттуда тип ShareData и импорта не менял.
 *
 *  Единственное отличие от прежнего файла — ГЛИФ ПРИЕЗЖАЕТ ССЫЛКОЙ, а не
 *  импортом `@muza/ui/assets/logo/glyph.svg`. Тот же приём, что у общего
 *  экрана входа (auth/LoginScreen, проп glyphSrc): сборщики двух площадок
 *  превращают импорт картинки в разное (Vite — в строку-URL, Next — в объект
 *  для next/image), и модуль, который собирается ОБОИМИ, не может позволить
 *  себе такую зависимость. Приложение отдаёт свой импортированный URL, веб —
 *  путь "/glyph.svg" из public/. */
 ...
 *  `glyphSrc` — ссылка на глиф Muza (см. шапку файла): у площадок она разная,
 *  поэтому обязательна и идёт ДО необязательного языка.
...
export async function renderShareCard(
  ...
  const glyph = await loadImage(glyphSrc);
  drawBase(ctx);
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
