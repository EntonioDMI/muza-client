...
export const VIS_LIMITS = {
  ...
  /** Скорость падения шапок-пиков. 50 воспроизводит прежние жёсткие числа
   *  (держать 0.8 с, ускорение 3.5) с точностью до знака — шкала строилась
   *  вокруг них, чтобы «как было» осталось достижимым.
   *
   *  ⚠️ ДЕФОЛТ 70, А НЕ 50. Прежнее поведение владелец назвал «падают очень
   *  медленно» при резко настроенном спектре (13.08): бары уже упали, а
   *  пластинки ещё висят, и вид прибора превращается в вид мусора. Числа
   *  audioMotion-analyzer, откуда они взяты, рассчитаны на плавные пресеты —
   *  под резкий спектр они не подходят. */
  peakFall: { min: 0, max: 100, def: 70 },
} as const;
...
export interface VisualizerTuning extends Record<VisLimitKey, number> {
  mirror: boolean;
  /** Рисовать ли шапки-пики вообще. Отдельно от скорости падения: «убрать
   *  насовсем» и «пусть падают быстрее» — разные желания, и сводить их к
   *  одному ползунку значит отнимать одно из них. */
  peaks: boolean;
}

/** Ползунок скорости падения → числа пик-холда.
 *
 *  Обе величины ведут в одну сторону (быстрее = короче пауза и злее
 *  ускорение), поэтому ручка одна: раздельные «пауза» и «ускорение» человек
 *  всё равно крутил бы вместе, а промахнуться ими врозь легко — короткая
 *  пауза при слабом ускорении даёт «сползание», которого никто не хочет.
 *  При fall = 50 отдаёт ровно прежние 0.8 с и 3.5. */
export function peakParams(fall: number): { holdSec: number; gravity: number } {
  const f = Math.max(0, Math.min(100, fall)) / 100;
  return { holdSec: 1.6 * (1 - f), gravity: 0.5 + f * 6 };
}
...
export function normalizeVisualizerTuning(raw?: Partial<VisualizerTuning>): VisualizerTuning {
  ...
  // Пики — ПО УМОЛЧАНИЮ ВКЛЮЧЕНЫ, поэтому сравнение с false, а не с true:
  // они существовали до появления ручки, и молчание вызывающего означает
  // «как было», а не «выключить». У mirror ровно наоборот, и перепутать эти
  // две строки — значит тихо погасить шапки у всех, кто ручку не трогал.
  out.peaks = raw?.peaks !== false;
  return out;
}
