...
export interface PlayerState {
   ...
   *
   *  Живёт рядом со `speed` не для порядка, а по смыслу: вместе они и есть то,
   *  что раньше выражалось одной ручкой. Пересчёт частоты (прежний путь)
   *  двигал время и высоту СЦЕПЛЕННО — медленнее значило ниже. Фазовый вокодер
   *  эту связь рвёт: можно замедлить, не опуская тон, и опустить тон, не
   *  замедляя.
   *
   *  0 — как записано. Работает только на нативном пути (Rust): у веба сдвига
   *  высоты нет, там ручка не показывается. */
  pitch: number;
}
...
const RANGES: Record<string, { min: number; max: number }> = {
  ...
  // ±2 октавы. Предел не от библиотеки, а от смысла: дальше фазовый вокодер
  // перестаёт звучать музыкой при любых настройках, и ход ручки становится
  // бесполезным. Зеркалит кламп в Rust (audio.rs, set_pitch) — правится ПАРОЙ.
  pitch: { min: -24, max: 24 },
};
...
/** Сбросить отложенную запись на диск НЕМЕДЛЕННО. В бою это делает обработчик
 *  ухода окна (pagehide); тестам нужен явный вызов, потому что «закрыл
 *  приложение» они изображают размонтированием, а окно склейки живёт таймером. */
export function flushPlayerState(): void {
  writeNow();
}

/** ЗАБЫТЬ ВСЁ, что модуль держит в памяти, вместе с отложенной записью.
 *
 *  Нужен тестам, и это не «удобство», а необходимость: `live`/`dirty` —
 *  состояние МОДУЛЯ, а модуль в прогоне один на весь файл. Очистка localStorage
 *  между тестами его не касается, и отложенная запись прошлого теста при первом
 *  же чтении в следующем воскресала бы в уже вычищенном хранилище. Ловилось это
 *  весело: повтор, включённый одним тестом, ломал авто-переход в трёх соседних —
 *  «следующий трек» оказывался тем же самым.
 *
 *  ⚠️ Не звать из приложения: человек не просил забывать его громкость. */
