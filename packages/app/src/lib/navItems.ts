 ...
 *
 *  Переехало из apps/desktop/src/lib/navItems.ts (волна «настройки»,
 *  2026-08-02) вместе с под-экраном «Навигация»: под-экран уехал в общий
 *  пакет, а тянуть из пакета в приложение запрещено. Модуль чистый — ни
 *  Tauri, ни DOM: ключи берутся из общей модели настроек (prefs/types),
 *  подписи — из общего словаря. На старом месте пенёк-ре-экспорт.
 *
 ...
 *  данные — не трогаем). Живое переключение языка делает navItemLabel ниже. */

import { DEFAULT_LANG, translate, type Lang } from "../i18n";
import { NAV_ITEM_KEYS, type NavItemKey } from "../prefs/types";
...
/** Локализованная метка вкладки: вместо статичного NAV_ITEM_META[key].label
 *  (EN-снимок) — `navItemLabel(key, prefs.language)`. */
