...
export const viewsEn = {
  ...
  admin: {
    ...
    // Листалка страниц: одна на всех (см. ru.views.ts)
    pager: {
      prev: "Previous page",
      next: "Next page",
      pageOf: "Page {page} of {pages}",
    },
    // Честность обрезаний (05.08): ни один урезанный список не молчит об этом
    limits: {
      shown: "Showing {count}",
    ...
    health: {
      ...
      recipeNote: "The server currently runs recipe v{version}. A new one arrives with the next server update — partial rollouts aren't possible yet.",
      versionCol: "Version",
    },
    users: {
      ...
      // Сортировка выключается, когда людей больше одной страницы (см. ru.views.ts)
      sortOffNote: "Sorting turns on once everyone fits on a single page.",
    },
    themes: {
    ...
    growth: {
      ...
      visitsNote: "Once a day the app sends an anonymous \"I'm running\" signal — only if telemetry is left on. It carries no name and no address.",
      ...
      tagCol: "Release",
      countCol: "Downloads",
      downloadsNote: "The download counter is read from GitHub every few hours; the chart shows the daily gain.",
    },
    errors: {
      ...
      sortByCount: "Most frequent",
      sortByLast: "Most recent",
      appFilterAll: "All versions ({count})",
      note: "The server scrubs error text: paths, tokens and emails are masked. Stacks are never collected — only a fingerprint, so identical errors land in one group.",
      clear: "Clear",
