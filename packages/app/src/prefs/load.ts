...
/** УЛИКИ С ДИСКА, которых сама модель настроек не знает.
 *
 *  mergePrefs обязана остаться ЧИСТОЙ: её тестируют напрямую, зовут из веба и
 *  из тестов с рукотворными профилями, и поход в localStorage изнутри сделал бы
 *  её результат зависимым от окружения. Улику собирает loadPrefs и передаёт
 *  сюда третьим параметром. */
export interface LoadEnv {
  /** Ключ `muza.resume.v1` СУЩЕСТВУЕТ в хранилище (см. ступень v4 ниже). */
  hadResumeHistory: boolean;
}

/** Карта позиций «продолжить с места». Читается здесь ТОЛЬКО как улика
 *  (существует/нет) — саму карту ведёт apps/desktop/src/lib/resumeStore.ts. */
...
export function mergePrefs(
  ...
  env: LoadEnv = { hadResumeHistory: false },
): Prefs {
  ...
  if (lookFrom < 4) {
    // ── «ПРОДОЛЖИТЬ С МЕСТА» ПО УМОЛЧАНИЮ ────────────────────────────────
    // Дефолт стал true (prefs/types.ts), но профиль хранится ЦЕЛИКОМ, поэтому у
    // каждого живого пользователя лежит явное false — смена дефолта его бы не
    // коснулась. Отличить «дефолт, которого никто не выбирал» от «выключил
    // осознанно» позволяет улика на диске, и она однозначна: ключ
    // muza.resume.v1 пишется ТОЛЬКО под условием resumePosition (единственный
    // писатель — resumeStore.save, и его вызов закрыт этим условием). Ключ
    // есть — настройку когда-то включали, сегодняшнее false осознанно, не
    // трогаем. Ключа нет — false досталось от дефолта, переводим.
    //
    // Проверяется СУЩЕСТВОВАНИЕ ключа, а не содержимое: карта позиций может
    // быть вычищена до `{}` (clear по всем трекам), но сам ключ остаётся.
    if (!env.hadResumeHistory) prefs.resumePosition = true;
    // ── ПРОФИЛЬ, ЗАСТРЯВШИЙ МЕЖДУ РАСКЛАДКАМИ ────────────────────────────
    // Отменённая ступень v3 успела поставить gapZone: 0 профилям, у которых
    // плоский вид ВЫКЛЮЧЕН. Такой профиль показывает зоны со скруглением и
    // впритык друг к другу, а вкладка раскладки утверждает «Воздушная» —
    // человек не нажимает уже выбранное и потому не чинится сам. Возвращаем
    // зазор воздушной раскладки.
    //
    // Обратную пару (zonesDocked true + gapZone > 0) НЕ трогаем: «плоский вид с
    // зазорами» можно собрать ползунком осознанно, улики против этого нет.
    if (prefs.zonesDocked === false && prefs.gapZone === 0) prefs.gapZone = 8;
  }
...
export function loadPrefs(base: Prefs = DEFAULT_PREFS): Prefs {
  try {
    ...
    // Улику собираем ЗДЕСЬ: mergePrefs остаётся чистой (см. LoadEnv).
    // getItem, а не разбор карты: значение неважно, важен сам факт, что ключ
    // когда-то писали.
    const prefs = mergePrefs(stored, base, { hadResumeHistory: localStorage.getItem(RESUME_KEY) !== null });
    // ── МИГРАЦИЯ ЗАКРЕПЛЯЕТСЯ НА ДИСКЕ, А НЕ ПЕРЕСЧИТЫВАЕТСЯ КАЖДЫЙ РАЗ ──
    //
    // ⚠️ Без этой записи ступень v4 ОТМЕНЯЛА САМА СЕБЯ, и «продолжить с места»
    // жило ровно один сеанс. Цепочка: сеанс 1 — ключа muza.resume.v1 нет, улик
    // нет, resumePosition становится true; человек слушает трек дольше пяти
    // секунд — resumeStore заводит этот ключ; сеанс 2 — улика теперь ЕСТЬ, и та
    // же ступень читает её как «выключил осознанно», а в сохранении по-прежнему
    // лежит false, потому что никто ничего не записал. Настройка выключается
    // сама, и виноватым выглядит приложение.
    //
    // Записываем ровно один раз — когда версия схемы в сохранении отличается от
    // текущей. Дальше `lookFrom` равен LOOK_VERSION, и ни одна ступень не
    // выполняется вовсе: миграция обязана быть одноразовой по смыслу.
    //
    // Побочно чинится давняя тихая беда ступени v2: она возвращала «нетронутые»
    // числа к новым дефолтам НА КАЖДОМ чтении, поэтому человек, осознанно
    // выставивший себе старое значение (зазор 12, бар 92), терял его при
    // следующем запуске снова и снова.
    //
    // Безопасность записи при чтении: у приложения loadPrefs зовётся только
    // после initDurableState() (main.tsx ждёт его перед первым рендером), то
    // есть localStorage к этому моменту уже поднят из файла-зеркала и мы не
    // затираем более свежую копию с диска.
    if (stored.lookVersion !== LOOK_VERSION) savePrefs(prefs);
    return prefs;
  } catch {
    return base;
  }
