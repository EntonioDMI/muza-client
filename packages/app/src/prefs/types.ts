...
export interface Prefs {
  ...
  accentSlider: string;
  accentActive: string;
  /** ОПОРНОЕ СКРУГЛЕНИЕ, px 0–40 — одно число на всю пятиступенчатую шкалу
   *  (--r-xs…--r-xl считает themeVars.ts::radiusScale из этого значения).
   *
   *  БЫЛО ТРИ ПРЕСЕТА mild/soft/round, стало свободное число (2026-08-13).
   *  Жалоба владельца дословно: «есть параметр „Скругление“, но нельзя
   *  полностью выключить скругление или выкрутить его на возможный максимум.
   *  Доступно всего три пресета… У пользователя есть три зашитых варианта,
   *  которые он не может изменить, пока не воспользуется плагинами».
   *  0 — совсем острые углы, 40 — предельно круглые; 8/16/28 отдают ровно то,
   *  что раньше давали mild/soft/round (три быстрых чипа над ползунком никуда
   *  не делись — частое остаётся в один клик).
   *
   *  Миграция сохранённых строк — той же дорогой, что у radiusTiles и прочих
   *  бывших пресетов (LEGACY_ENUM_TO_NUMBER в prefs/legacyPrefs.ts). */
  radius: number;
  /** Скругление по типам ПОВЕРХ опорного: плитки/строки и панели — ПРОЦЕНТ
   *  (0–200, 100 = как опорное; 0 = острые углы, 200 = супер-круглые);
   *  кнопки, поля и вкладки — px 0–26 либо RADIUS_OVERRIDE_OFF (999) = «Авто»
   *  (токен --r-control/--r-field/--r-tabs не ставится, и форму задаёт общее
   *  скругление — см. блок «форма органов управления» в tokens/radius.css).
  ...
  /** СТЕКЛО ЦЕЛИКОМ: включено (дефолт) или сплошные поверхности.
   *
   *  Заявка владельца 2026-08-13: «было бы классно, если бы в кастомизации
   *  можно было настроить элементы интерфейса — допустим, полностью выключить
   *  „стекло“». Ползунок «Прозрачность» этого не умел и уметь не мог: лестница
   *  материалов даже на 100 % оставляет зону полупрозрачной нарочно (стекло,
   *  ставшее глухим, перестаёт быть стеклом — см. MATERIAL в themeVars.ts), и
   *  размытие при этом никуда не девается.
   *
   *  Выключено — материалы становятся сплошными, размытие панелей гаснет
   *  (размывать под глухой заливкой нечего — это плата ни за что), плёнки
   *  элевации плотнеют, потому что роль разделителя зон переходит от
   *  прозрачности к плотности. Это ещё и САМАЯ ДЕЙСТВЕННАЯ ручка
   *  производительности в программе: backdrop-filter заставляет композитор
   *  снимать и размывать отдельный слой под каждой панелью.
   *
   *  Системное «меньше прозрачности» действует независимо от этого тумблера и
   *  сильнее его (tokens/glass.css) — просьбу доступности своей темой не
   *  отменяют. */
  glassOn: boolean;
  ...
  /** Шапки-пики над барами: рисовать или нет. Существовали жёстко с самого
   *  появления баров — ручка заведена 13.08 по жалобе «настроить нельзя». */
  visualizerPeaks: boolean;
  /** Скорость падения шапок, % (0–100). 50 — прежние жёсткие числа. */
  visualizerPeakFall: number;
   ...
   *  ВКЛЮЧЕНО ПО УМОЛЧАНИЮ с 2026-08-05: выключенным оно было не решением, а
   *  осторожностью первых сборок, и человек, закрывший приложение посреди
   *  часового микса, терял место — при том что механика давно готова и
   *  проверена. Существующие профили переводит ступень v4 (prefs/load.ts), и
   *  только те, кто настройку никогда не включал, — см. улику там же. */
  resumePosition: boolean;
  /** Характер растяжения темпа: true — WSOLA, false — фазовый вокодер.
   *
   *  Это ВКУС, а не качество. Вокодер раскладывает звук на частоты и гладок на
   *  тянущемся — вокал, синты, струнные. WSOLA работает во временной области и
   *  держит УДАРЫ: он не пересчитывает атаку, а подбирает точку склейки так,
   *  чтобы форма волны совпала. На битовой музыке разница слышна сразу (жалоба
   *  друга владельца 11.08.2026: «звук не такой, как в Music Speed Changer» —
   *  там ровно так же переключаются два обработчика).
   *
   *  Здесь, а не в памяти плеера: playerState хранит ПОЛОЖЕНИЯ ОРГАНОВ
   *  управления (громкость, повтор, скорость), а это решение о ПОВЕДЕНИИ
   *  программы — ему место в настройках, рядом с кроссфейдом и нормализацией.
   *
   *  Разбор алгоритма — apps/desktop/src-tauri/src/wsola.rs. */
  tempoWsola: boolean;
  ...
  /** Ручная поправка синхронного текста, мс (−3000..+3000; плюс — раньше).
   *  Поверх постоянного упреждения LYRICS_LEAD_MS (lib/activeLine.ts): то
   *  лечит нашу задержку показа, эта — враньё самих таймкодов у источника
   *  (в LRCLIB попадаются тексты, размеченные ровной сеткой «строка каждые
   *  2 секунды», и одна разметка, разложенная на издания разной длины).
   *  Тема его не носит (подстройка руками, а не внешность), а МЕЖДУ
   *  УСТРОЙСТВАМИ он ездит: правит он в основном чужие таймкоды, одинаковые
   *  везде, а разница в латентности вывода двух машин меньше шага ползунка
   *  (50 мс) и человеком не слышна. */
 ...
 *  3 — СОЖЖЁННЫЙ НОМЕР (флет-дефолт, развёрнутый в тот же вечер; профили с
 *  этой меткой существуют — см. LOOK_V3_MOVES в prefs/load.ts),
 *  4 — «продолжить с места» по умолчанию + починка профилей, застрявших между
 *  раскладками после отмены v3 (зоны с зазором 0 при невключённом плоском виде).
 *  Поднимается только вместе с новой ступенью в prefs/load.ts. */
export const LOOK_VERSION = 4;
...
export const DEFAULT_PREFS: Prefs = {
  ...
  accentSlider: "#3b82f6",
  accentActive: "#3b82f6",
  // 16 = бывший пресет "soft", число в число (tokens/radius.css)
  radius: 16,
  radiusTiles: 100,
  radiusPanels: 100,
  ...
  // «Классика» ставит 12.
  gapZone: 8,
  // Onest с 11.08.2026 (было "golos"). ⚠️ Меняется ТРОЙКОЙ: здесь, первым
  // элементом FONT_CHOICES и условием в themeVars.ts — иначе дефолт начнёт
  // ставить переменную поверх самого себя.
  fontUi: "onest",
  ...
  visualizerPeaks: true,
  // 70, а не прежние 50: при резко настроенном спектре бары успевали упасть,
  // а пластинки ещё висели — владелец назвал это «падают очень медленно».
  visualizerPeakFall: 70,
  ...
  lyricsEndNote: true,
  lyricsOffsetMs: 0, // ноль: упреждение уже зашито в activeLine, рукой — по вкусу
