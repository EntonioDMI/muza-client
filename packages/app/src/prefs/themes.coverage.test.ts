 ...
 *
 *  ПЕРЕЕЗД 2026-08-02 (волна веб-паритета): сторож приехал из
 *  apps/desktop/src/lib/themes.coverage.test.ts ВМЕСТЕ с моделью. Теперь у него
 *  вторая, более важная работа: классификация обязана покрывать ровно
 *  DEFAULT_PREFS, поэтому второй копии Prefs завести нельзя — она немедленно
 *  разъедется с THEME_KEYS/THEME_EXCLUDED и уронит сборку. Именно из-за этого
 *  сторожа экран настроек ждал переезда модели. */
...
describe("классификация ключей Prefs для тем", () => {
  ...
  // Мета-проверка самого сторожа (2026-08-02): «тест зелёный» и «тест умеет
  // краснеть» — разные утверждения, а этот сторож теперь единственное, что
  // держит модель настроек в одном экземпляре. Здесь имитируется ровно та
  // ошибка, ради которой он живёт: в Prefs появилось поле, которое забыли
  // отнести к оформлению или к поведению.
  it("сторож РЕАЛЬНО ловит неклассифицированное поле", () => {
    const withNewField = { ...DEFAULT_PREFS, someBrandNewPref: 1 };
    const all = Object.keys(withNewField).sort();
    const classified = [...THEME_KEYS, ...THEME_EXCLUDED].sort();
    expect(classified).not.toEqual(all);
  });
});
