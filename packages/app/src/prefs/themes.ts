...
export const THEME_KEYS = [
  ...
  "bassSharp",
  "bassReach",
  // РАЗМЕРЫ ЗОН ОСТАЮТСЯ ОФОРМЛЕНИЕМ и едут с темой — в отличие от силуэта
  // окна (playerDocked/zonesDocked/gapZone, см. THEME_EXCLUDED). Граница
  // проведена так намеренно, не наугад: «полоса плеера высотой 92 и обложка
  // 60» — это пропорции, которые автор темы задумал и без которых его тема
  // выглядит не так; «плеер прижат ко дну окна» — это РАСКЛАДКА, отдельная
  // ось настроек со своим рядом на экране «Внешний вид».
  "hPlayerBar",
  "coverBarSize",
  "hProgress",
  "tileSize",
...
export const THEME_EXCLUDED = [
  ...
  "lyricsAutoScroll",
  "lyricsEndNote",
  // Сдвиг текста — калибровка под КОНКРЕТНУЮ машину (латентность вывода) и под
  // кривые таймкоды конкретных песен. Чужая тема, приехавшая с чужим сдвигом,
  // сбила бы человеку то, что он подстроил руками.
  "lyricsOffsetMs",
  "videoNowPlaying",
  ...
  "scrollSpeed",
  "scrollSmooth",
  // ── СИЛУЭТ ОКНА (2026-08-05, решение владельца) ─────────────────────────
  // «Не стоило добавлять воздух и классику в тему, это совсем не подходит»:
  // раскладка окна — отдельная ось со своим рядом на экране «Внешний вид», и
  // чужая тема не имеет права её переставлять. Пока эти три ключа ехали с
  // темой, любая применённая тема молча перекладывала окно: плавающий плеер
  // прижимался ко дну, зоны теряли зазор — а человек искал причину в своих
  // настройках раскладки, где ничего не менялось.
  // ⚠️ ГРАНИЦА: сюда уходит только силуэт. Размеры (hPlayerBar, coverBarSize,
  // wSidebar, tileSize) остаются в THEME_KEYS — см. комментарий там.
  "playerDocked",
  "zonesDocked",
  "gapZone",
  // ── ХАРАКТЕР ДВИЖЕНИЯ (2026-08-05, подтверждено живым чтением профиля) ──
  // Пока эти пять ключей ехали с темой, ЛЮБАЯ применённая тема молча меняла
  // скорость и кривые анимаций всего приложения. Владельцу так и приехал
  // easeStyle: "crisp" — он его не выбирал и найти причину не мог: ряд «Скорость
  // анимаций» показывал значение, которого человек не ставил.
  // Движение — это ощущение отклика программы, а не её раскраска: тема вправе
  // перекрасить окно, но не вправе решать, насколько резко оно отвечает.
  "animSpeed",
  "durMenuMult",
  "durDialogMult",
  "durPageMult",
...
export const THEME_NUMBER_RANGES: Record<string, NumberRange> = {
  ...
  visualizerPeakFall: VIS_LIMITS.peakFall,
