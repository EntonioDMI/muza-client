 ...
 *
 *  ПЕРЕЕЗД 2026-08-02 из apps/desktop/src/lib/fonts.ts — и почему в prefs/,
 *  а не в theme/: это РЕЕСТР ДОПУСТИМЫХ ЗНАЧЕНИЙ полей Prefs.fontUi/fontDisplay
 *  (какие ключи вообще бывают и что каждый значит), то есть часть модели.
 *  В theme/ живёт слой ПРИМЕНЕНИЯ — accent.ts, themeVars.ts, ThemeRoot.tsx;
 *  он этот реестр читает (themeVars.fontFamily теперь делегирует сюда, второго
 *  списка шрифтов в вебе больше нет).
 *
 *  Бандлятся у клиентов РАЗНЫЕ подмножества (@fontsource в приложении, две
 *  семьи в вебе). Реестр общий намеренно: незагруженный шрифт браузер сам
 *  роняет в хвост TAIL, а вот второй список расходился бы молча. */
