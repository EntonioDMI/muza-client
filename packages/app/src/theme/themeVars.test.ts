...
import { GLASS_MIN } from "../views/settings/primitives";
...
describe("buildThemeVars — дефолтный профиль", () => {
  ...
  it("стекло и текст считаются из процентов профиля", () => {
    ...
    expect(v["--text-3"]).toBe("rgba(244, 243, 241, 0.57)");
  });
 ...
 *  весь ход ползунка стекла — от GLASS_MIN до 100, границу берём ИЗ КОНСТАНТЫ.
 *  ⚠️ Здесь стояло «30–100», и это было неправдой с тех пор, как GLASS_MIN
 *  опустили до 0 (primitives.tsx:41): сторож обещал весь ход, а мерил верхние
 *  70 %. Полоса 0–29 была достижима человеком и не проверялась вовсе; на нуле
 *  там лежало 4.49:1 при норме 4.5. Число брать из константы, не переписывать
 *  руками — иначе разъедется снова.
 *
...
describe("стеклянная лестница — читаемость", () => {
  ...
  const worst = (dim: number) => {
    ...
    for (const theme of ["dark", "light"] as const) {
      for (let pct = GLASS_MIN; pct <= 100; pct += 2) {
  ...
  it("даже с выключенным затемнением фона не проваливается ниже 3.9:1", () => {
    ...
    //
    // ⚠️ Порог опущен 4.0 → 3.9 (15.08) РОВНО ОДИН РАЗ и не потому, что стало
    // хуже. Сторож раньше перебирал ползунок стекла с 30, хотя GLASS_MIN давно
    // равен 0: числа 4.07 и 4.0 измерены на суженном ходе. На настоящем ходе
    // худшее — 3.96:1 (dark, стекло=0 %, панель/surface-4/--text-3), и это уже
    // ПОСЛЕ подъёма чернил (шаг тёмной темы 0.06 → 0.05). Норму AA 4.5 при этом
    // не трогали: она проходит на всём ходу. Здесь смягчена только собственная
    // страховка, и смягчена до измеренного факта, а не «с запасом».
    const low = worst(0);
    expect(low.c, `худшее ${low.c.toFixed(2)}:1 — ${low.where}`).toBeGreaterThanOrEqual(3.9);
  });
 ...
 *  0.57 (шаг 0.05 — сжат волной «больше стекла», затем ещё раз 15.08, когда
 *  сторож лестницы пошёл по всему ходу ползунка стекла; разбор у --text-3 в
...
describe("третий тон текста", () => {
  it("держит шаг своей темы на всём ходу ручки", () => {
    for (const [theme, step] of [["dark", 0.05] as const, ["light", 0.12] as const]) {
