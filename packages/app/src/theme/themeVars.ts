...
export function buildThemeVars(input: ThemeInput, stage: ThemeStage = {}): CSSProperties {
  ...
  // Шаг тёмной темы 0.06 → 0.05 (15.08). Сторож стал перебирать ВЕСЬ ход
  // ползунка стекла (GLASS_MIN = 0, а не 30, как было записано в его же
  // комментарии) и на нуле поймал 4.49:1 при норме 4.5 — связка
  // панель/surface-4/--text-3 в тёмной теме. Плотностью это не чинится:
  // подъём zone и panel просто передаёт узкое место друг другу (замерено —
  // обе дают те же 4.49) и ломает совпадение инлайна с фолбэками glass.css.
  // Это ровно тот случай, который описан у токена --text-3 в
  // packages/ui/src/tokens/colors.css: светлеющая плёнка элевации поднимает
  // фон поверх любого стекла, и рычаг остаётся один — чернила.
  const textAlpha3 = Math.max(0.2, textAlpha2 - (isLight ? 0.12 : 0.05));
