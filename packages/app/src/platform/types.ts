...
/** Итог одного разбора файлов/папки.
 *
 *  Раньше метод возвращал просто массив, и ТРИ разных исхода приходили
 *  неотличимо пустыми: «в папке правда нет музыки», «файлы есть, но ни один не
 *  читается» и «файлов слишком много, взяли часть». Человек на все три видел
 *  один тост «в папке не нашлось аудиофайлов» — отсюда впечатление, что
 *  локальная библиотека не работает вовсе (жалоба 12.08). */
export interface LocalScanResult {
  entries: LocalFileEntry[];
  /** Сколько подходящих файлов увидел обход — ДО чтения тегов.
   *  Больше `entries.length` = столько-то файлов не открылись как аудио. */
  found: number;
...
export interface LocalFilesPort {
  ...
  pickAndScan(kind: "files" | "folder"): Promise<LocalScanResult | null>;
  /** Разбор готового списка путей (файлы бросили в окно). */
  scanPaths(paths: string[]): Promise<LocalScanResult>;
