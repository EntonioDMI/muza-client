    ...
    >
      ...
      <div
        style={{
          ...
          /* ⚠️ ПОСТОЯННЫЙ center, А НЕ ПЕРЕКЛЮЧАЕМЫЙ (13.08). Раньше стояло
             `lyricsShown ? undefined : "center"`, и это ломало ПОКАЗ текста:
             justify-content не интерполируется, он щёлкает на первом же кадре.
             При показе обложка мгновенно прыгала из центра влево, а колонка
             только потом разъезжалась — владелец описал это как «появляется
             резко, а скрывается с анимацией». При скрытии тот же щелчок
             совпадал по направлению со схлопыванием и был не виден, отсюда и
             асимметрия.
             Держать его включённым всегда безопасно: при показанном тексте
             второй трек 1fr съедает всю свободную ширину, центрировать нечего,
             и center ни на что не влияет. Вся анимация в обе стороны идёт
             через сам трек 1fr → 0fr. */
