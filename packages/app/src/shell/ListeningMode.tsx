 ...
 *
 *  ⚠️ ЦЕНА КАДРОВ. Здесь ДВА rAF-цикла, и оба обязаны молчать при закрытом
 *  оверлее: качание при басах (эффект ниже выходит по !open) и визуализатор
 *  (ему передаётся active={open}, подробности — в шапке shell/Visualizer.tsx).
 *  Владелец жаловался 02.08, что караоке роняет ФПС в играх; постоянный
 *  потребитель там — визуализатор 60 Гц. Оверлей НЕ размонтируется при
 *  закрытии (он гаснет прозрачностью, чтобы возврат был плавным), поэтому
 *  «закрыт» здесь означает именно «циклы остановлены», а не «нет в DOM».
 *
