...
function trackItems(target: Extract<ContextTarget, { kind: "track" }>, ctx: MenuAbilities, t: T): MenuItem[] {
  ...
  return [
    ...
    ...(place !== "player" && ctx.queueTrack
      ...
      : []),
    ...(ctx.startRadio ? [{ icon: "radio", label: t("menu.catalog.radio"), onClick: () => ctx.startRadio?.(tr) }] : []),
    ...(ctx.addToPlaylist
...
function playlistItems(pl: { id: string; name: string }, ctx: MenuAbilities, t: T): MenuItem[] {
  ...
  const card: MenuItem[] = [
    ...(ctx.sharePlaylist ? [{ icon: "share-2", label: t("menu.catalog.share"), onClick: () => ctx.sharePlaylist?.(pl.id) }] : []),
