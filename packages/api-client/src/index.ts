...
export interface MuzaApi {
  ...
  /** ГДЕ ИЩЕТСЯ МУЗЫКА И КТО ИЗ НИХ СЕЙЧАС МОЛЧИТ (GET /health/sources).
   *
   *  Зачем это в контракте вообще: 13.08 выяснилось, что одно из мест поиска
   *  может лежать неделями, а человек видит лишь однобокую выдачу и считает её
   *  нашим решением. Экран проверки без этих цифр отвечает только за то, что
   *  происходит НА УСТРОЙСТВЕ, и слеп ровно к той половине, где авария и жила.
   *
   *  ⚠️ Метод НЕОБЯЗАТЕЛЬНЫЙ (по образцу DiagnosticsPort.startSummary): нет
   *  умения — на экране нет и раздела, а не пустая рамка с прочерками. Так же
   *  это оставляет в покое места, которые собирают свой объект api руками
   *  (тесты и веб-заглушки): добавь метод обязательным — и они перестанут
   *  собираться, ничего при этом не получив. */
  searchSourceHealth?(): Promise<SearchSourceHealth[]>;
