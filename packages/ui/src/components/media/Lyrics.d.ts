...
export interface LyricsProps {
  ...
  endNote?: boolean;
  /** Караоке: сколько строк видно в окне вокруг активной. Окно симметрично,
   *  поэтому число нечётное (3, 5, 7…); 5 — прежнее поведение. Работает только
   *  при включённом автоследовании: в ручном режиме виден весь текст. */
  windowLines?: number;
  /** Панель: 0 — «Авто» (размер строки от общего «Размера текста»), N ≥ 2 —
   *  подобрать размер строки так, чтобы в панель влезло ровно N строк. */
  panelLines?: number;
