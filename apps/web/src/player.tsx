...
export function PlayerProvider({ children }: { children: React.ReactNode }) {
  ...
  /** ⚠️ СОСТОЯНИЕ ДЛЯ СИСТЕМЫ — ОБЯЗАТЕЛЬНОЕ, А НЕ УКРАШЕНИЕ.
   *
   *  Жалоба 11.08 (iPhone + AirPods): «поставить на паузу с наушников могу, а
   *  возобновить — нет; приходится лезть в приложение и жать продолжить».
   *
   *  Причина: без `playbackState` система держит СВОЁ представление о том,
   *  играет ли музыка, и после паузы с наушников оно остаётся «играет». Второе
   *  нажатие пульта поэтому шлёт снова `pause` — ту команду, которую мы
   *  исправно исполняем, — и со стороны это выглядит как «play не работает».
   *  Обработчик `play` при этом был на месте и был исправен: его просто никто
   *  не звал.
   *
   *  Приложение эту строку имеет с самого начала
   *  (apps/desktop/src/player/useMediaSession.ts) — в вебе её не было. Ещё одно
   *  расхождение из того же семейства, что рельс настроек: две реализации
   *  одного, обновили одну.
   *
   *  Отдельным эффектом от обработчиков: те переставляются на смену трека, а
   *  состояние обязано ехать за КАЖДЫМ пуском и паузой, откуда бы они ни
   *  пришли — из интерфейса, с наушников или от самого элемента. */
  useEffect(() => {
    if (!("mediaSession" in navigator) || !prefs.mediaKeys) return;
    navigator.mediaSession.playbackState = playing ? "playing" : "paused";
  }, [playing, prefs.mediaKeys]);
