...
function Player({
  ...
  // забирает ширину.
  // ⚠️ ЭКРАН ЗДЕСЬ — `rendered`, А НЕ `view` (жалобы владельца 13.08: «переход
  // на Главную — всё прыгает по анимациям» и «вход в плейлист из библиотеки —
  // на заднем фоне элементы расплываются, увеличиваются, багуются и дёргаются»).
  //
  // ЧТО БЫЛО. `view` меняется В МОМЕНТ ЩЕЛЧКА, а нарисован в это время ещё
  // ПРЕЖНИЙ экран — он доигрывает своё затухание (машина — useViewTransition).
  // То есть колонка под панель появлялась/исчезала ПОД живым содержимым
  // уходящего экрана. Замер (окно 1600px, Библиотека → Главная, кадры CDP
  // screencast, серия из 16): ширина СОДЕРЖИМОГО main падала 960 → 612 одним
  // кадром и потом ~200 мс ползла обратно к 960, всё это время фаза была `out`,
  // то есть библиотека была на экране и видимо переставлялась. На пустой
  // Библиотеке это читается как съезд блока на 89px за 92 мс; на сетке плиток —
  // как смена числа колонок, то есть «плитки увеличиваются и дёргаются».
  // Переход БЕЗ смены типа экрана (Библиотека → Поиск) в том же замере не менял
  // ширину вовсе — отсюда и «при переходе не на главную такого нет».
  //
  // КАК СТАЛО. Резервирование места привязано к НАРИСОВАННОМУ экрану, поэтому
  // геометрия меняется ровно в том коммите, где поддерево ремонтируется:
  // уходящий экран доживает в своей ширине, приходящий монтируется сразу в
  // своей. Ширина содержимого при навигации теперь не меняется ВООБЩЕ — пара
  // «колонка сетки ↔ поле main» подобрана так, что 960 остаётся 960 (проверено
  // тем же замером после правки).
  const nowPlayingDocked =
    (showNowPlaying || npClosing) &&
    (rendered === "home" || rendered === "playlist" || rendered === "scPlaylist" || rendered === "favorites");
  ...
  return (
    ...
    {/* data-radius снят 13.08: скругление стало свободным числом, и атрибут
        рисовал мёртвое data-radius="16" — селекторов под него не осталось ни
        одного (форму задают токены --r-*, которые считает themeVars). */}
    <div data-theme={prefs.theme} data-accent={accentAttr} data-muza-layer-root="" style={rootStyle}>
      ...
      >
        ...
        <main
          ...
          style={{
            ...
            //
            // ⚠️ ПОЛЕ МЕНЯЕТСЯ ОДНИМ КАДРОМ И НЕ АНИМИРУЕТСЯ. Здесь стоял
            // переход по этому самому полю длиной --dur-panel-in («плавно, чтобы
            // открытие панели не дёргало список»), и он был ПРИЧИНОЙ
            // жалобы владельца 13.08 «открываю „Сейчас играет“ — треки на фоне
            // исчезают, появляются, снова исчезают и проявляются только в конце
            // анимации панели».
            //
            // padding-right — свойство РАСКЛАДКИ. Его анимация заставляет
            // браузер пересчитывать раскладку и перекрашивать ВЕСЬ список на
            // каждом кадре (web.dev, «Animations and performance»: анимируем
            // только transform и opacity). Замер живого окна (CDP screencast,
            // экран Поиска): открытие панели — 11 разных ширин содержимого за
            // 275 мс, закрытие — 14 за 470 мс. Каждая из них — полный reflow
            // сетки; на длинном списке с обложками кадр не укладывается в
            // бюджет, часть кадров пропадает, и содержимое мерцает. Это и есть
            // «исчезает и появляется».
            //
            // Теперь место освобождается за один коммит, а движется только сама
            // панель — transform и opacity, композитор, ноль пересчётов
            // раскладки. Ровно так уже вело себя слушательское расположение
            // (там место даёт колонка сетки, и она никогда не анимировалась) —
            // то есть правка ещё и УБИРАЕТ разное поведение одного действия на
            // разных экранах.
