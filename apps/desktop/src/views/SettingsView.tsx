...
export function SettingsView({
  ...
  /** Номер записи истории. Растёт ТОЛЬКО на «назад»/«вперёд» — по нему экран
   *  и отличает переход по истории от собственного монтирования. */
  placeNonce?: number;
  ...
  // ⚠️⚠️ КРИТИЧЕСКАЯ ПРАВКА 13.08, ВЕЧЕР. Первая версия этой связки ВЕШАЛА
  // ИНТЕРФЕЙС. Жалоба владельца: «был на странице диагностики, перешёл на
  ...
  //
  // Механизм. Поддерево экрана пересоздаётся при каждой смене вкладки, то есть
  // SettingsView монтируется заново с tab = null. При этом App всё ещё держал
  // ПРОШЛОЕ место (диагностику). Дальше два эффекта начинали тянуть состояние в
  // разные стороны: эффект-приведение вёл экран к диагностике, а эффект-отчёт
  // на монтировании докладывал наверх (null, null) — App записывал это к себе,
  // приведение видело новое значение и вело обратно. Бесконечный цикл, который
  // человек и увидел как мигание между корнем и диагностикой.
  //
  // Разводит их НОМЕР ЗАПИСИ. Место применяется, только когда номер сменился, —
  // а номер растёт ТОЛЬКО на «назад»/«вперёд». Монтирование, обычный щелчок и
  // повторный отчёт номер не двигают, и приведение молчит. Приём не выдуман
  // здесь: ровно так же устроена заявка на под-экран (SettingsIntent.nonce
  // выше) — и по той же причине, «сделай один раз, а не каждый рендер».
  ...
  useEffect(() => {
    ...
    setTab((placeTab ?? null) as SettingsTabKey | null);
    setSub((placeSub ?? null) as SettingsSubKey | null);
  }, [placeNonce, placeTab, placeSub]);

  // Отчёт наверх — только о СМЕНЕ места человеком, и не о первой отрисовке.
  // Отчёт на монтировании был второй половиной зависания: он затирал в App
  // место, к которому мы ещё даже не пришли.
