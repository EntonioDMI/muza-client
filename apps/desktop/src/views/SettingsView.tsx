    ...
    sub === "customize" ? (
      ...
      <div key="playback" className={paneClass} style={paneStyle}>
        ...
        <SettingRow
          ...
          hint={
            prefs.crossfade
              ? "Стык без паузы вместо обычного переключения. Пока включён «Кроссфейд» выше — работает он, Gapless не участвует"
              : "Следующий трек стартует без паузы; последние ~50мс текущего трека при этом не звучат — обычно незаметно. Если окно свёрнуто, стык иногда откатывается на обычное переключение (без обрезки — просто не gapless в этот раз)"
          }
        >
