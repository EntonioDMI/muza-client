...
export function usePlayback({
  ...
  const engine = () => {
    if (!engineRef.current) {
      ...
      // подхватит их в предгреве или в ensureGraph — смотря что случится
      // раньше). Конфиг голоса — тем же приёмом.
  ...
  return useMemo(
    ...
    // ⚠️ ЗДЕСЬ ДОЛЖНО БЫТЬ ПЕРЕЧИСЛЕНО КАЖДОЕ useState ЭТОГО ХУКА (13.08.2026).
    //
    // Памятка отдаёт наружу СНИМОК состояния плеера: значения, которых нет в
    // этом списке, застывают на экране в том виде, в каком их застал последний
    // пересчёт. Внутри хука всё при этом работает — сеттер меняет состояние и
    // доносит его до движка, — поэтому дефект выглядит как «ручка не крутится»,
    // а не как «функция сломана», и тестами на движок не ловится вовсе.
    //
    // Именно это и случилось с высотой тона: `pitch` в списке отсутствовал, и
    // ползунок в меню «⋯» получал неизменный value — не двигался и держал
    // подпись «0», хотя вокодер честно отрабатывал каждое движение мыши.
    // Жалоба владельца звучала «высоту регулировать нельзя, а скорость можно» —
    // ровно потому, что `speed` в списке был. Регрессия — в usePlayback.test.ts
    // («высота тона видна снаружи хука»).
    //
    // Списка не избежать (exhaustive-deps выключен намеренно: сюда попали бы
    // все функции хука и памятка потеряла бы смысл), поэтому правило простое —
    // ЗАВЁЛ useState, ДОБАВЬ ЕГО СЮДА. Исключение ровно одно и осознанное:
    // позиция, она живёт в posStore и в объект не входит (разбор выше).
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [queue, track, index, playing, buffering, vol, speed, pitch, repeat, shuffle, outputDevices],
  );
}
