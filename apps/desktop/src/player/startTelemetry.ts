 ...
 *
 *  ⚠️ КЛАСС ДОСТАВЛЯЕТСЯ ЗДЕСЬ, а не у потребителя. Поле `cls` в записи
 *  необязательное: его ставит markClass, а если никто не позвал — класс
 *  выводится из отметок добычи и пути (startProvider). Раньше вывод жил только
 *  в сводке, и всякий, кому нужно было «откуда играл этот трек», выводил его
 *  сам — то есть заводил вторую копию правила. Разрешаем ОДИН раз, на выходе
 *  наружу: дальше по коду `cls` всегда осмыслен.
 *
 *  Записи пересобираются, а не правятся на месте: `[...ring]` копирует массив,
 *  но не сами записи, и дописывание поля испортило бы кольцо (а следом и то,
 *  что уедет на диск). Новый объект заводится только там, где класса не было. */
export function getStartLog(): StartRecord[] {
  ensureRestored();
  return [...ring].reverse().map((r) => (r.cls ? r : { ...r, cls: startProvider(r) }));
}
