...
describe("useJam — гость решает про seek по своей живой позиции", () => {
  ...
  it("настоящий разъезд — догоняем", async () => {
    ...
    // ⚠️ С ДОПУСКОМ, А НЕ ПОБАЙТОВО (уронило CI 12.08: ждали 100, пришло
    // 100.001). Позиция догона считается ОТ ЧАСОВ:
    // expectedPos = posSec + (Date.now() − updatedAt) / 1000 (useJam.ts).
    // Тест проставляет updatedAt при сборке события, а хук читает Date.now()
    // уже при его разборе — на медленной машине между этими двумя вызовами
    // проходит миллисекунда, и ровная сотня превращается в 100.001.
    // Это ПРАВИЛЬНОЕ поведение продукта: он компенсирует возраст снимка хоста.
    // Точное равенство здесь проверяло бы скорость машины, а не логику догона.
    expect(seek).toHaveBeenCalledTimes(1);
    expect(seek.mock.calls[0][0]).toBeCloseTo(100, 1);
  });
});
