...
/** ПРЯМАЯ РЕГРЕССИЯ на жалобу владельца (13.08.2026): «там, где три точки,
 *  высоту тона регулировать нельзя, а скорость можно».
 *
 *  Причина была не в ползунке и не в Rust, а в СПИСКЕ ЗАВИСИМОСТЕЙ useMemo,
 *  которым хук отдаёт наружу свой объект: `pitch` в нём отсутствовал, хотя
 *  `speed` был. Значение внутри хука менялось (и до движка доезжало), а вот
 *  `pb.pitch` оставался тем, каким его застал последний пересчёт памятки —
 *  ползунок в меню получал НЕИЗМЕННЫЙ value, не двигался и держал подпись «0».
 *  Отсюда и асимметрия в жалобе: у скорости зависимость была на месте.
 *
 *  ⚠️ ПРОВЕРЯЕМ СНАРУЖИ ХУКА, а не «сеттер позвал движок». Тест на движок был
 *  бы зелёным и на сломанном коде: движок-то значение получал. Ломается ровно
 *  то, что видит экран, — значит смотреть надо туда же, куда смотрит плеер-бар:
 *  в поле возвращённого объекта. */
describe("высота тона видна снаружи хука", () => {
  it("setPitch меняет pb.pitch, а не только внутреннее состояние", () => {
    const hook = mount();
    expect(hook.result.current.pitch).toBe(0);

    act(() => {
      hook.result.current.setPitch(3);
    });
    expect(hook.result.current.pitch).toBe(3);

    act(() => {
      hook.result.current.setPitch(-5);
    });
    expect(hook.result.current.pitch).toBe(-5);
  });

  it("cyclePitch (кнопка в баре) тоже обновляет видимое значение", () => {
    const hook = mount();
    act(() => {
      hook.result.current.cyclePitch(); // 0 → −1 по PITCH_STEPS
    });
    expect(hook.result.current.pitch).toBe(-1);
  });

  it("высота переживает перезапуск приложения", () => {
    const first = mount();
    act(() => first.result.current.setPitch(-2));
    first.unmount();
    flushPlayerState();

    expect(mount().result.current.pitch).toBe(-2);
  });
});
