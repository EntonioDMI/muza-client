...
function diskHas(env: { seq: number; value: string } | null) {
...
const diskWrites = () =>
  h.invoke.mock.calls
    ...
    .map(([, a]) => JSON.parse((a as { value: string }).value) as { seq: number; value: string });
...
describe("зеркалирование записей после init", () => {
  it("setItem зеркалится в файл с растущим номером; removeItem удаляет файл и счётчик", async () => {
    diskHas(null);
    await initDurableState();
    h.invoke.mockClear();
    h.invoke.mockResolvedValue(undefined);
    ...
    expect(localStorage.getItem(SEQ)).toBe(null);
    expect(h.invoke).toHaveBeenCalledWith("state_del", { key: KEY });
  });
