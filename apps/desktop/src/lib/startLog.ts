 ...
 *
 *  ⚠️ ПРЕДЕЛ ДЕРЖИТСЯ В ДВУХ МЕСТАХ, потому что за один старт добыча
 *  отчитывается НЕСКОЛЬКО раз (stream_start + resolve). Здесь режется один
 *  ответ, а накопление — в markTimings (startTelemetry.ts). Без второго среза
 *  в памяти копилось больше, чем переживало перезапуск: после чтения с диска
 *  запись молча менялась. */
export const TIMINGS_MAX = 32;
