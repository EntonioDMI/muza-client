...
/// Потолок ожидания байтов растущего файла. Разбор — GrowingSource::wait_for.
const GROWING_WAIT_MAX: std::time::Duration = std::time::Duration::from_secs(10);
                ...
                loop {
                    ...
                    move |out: &mut [f32], _: &cpal::OutputCallbackInfo| {
                        ...
                        if cb.paused.load(Ordering::Relaxed) {
                            out.fill(0.0);
                            // ⚠️ ВИЗУАЛИЗАТОР ОБЯЗАН ЗАТУХНУТЬ, А НЕ ЗАМЕРЕТЬ.
                            // Кольцо снимка наполняется ниже, в обычной ветке, и
                            // этот ранний return оставлял в нём последние 2048
                            // сэмплов ДО паузы. native_scope считал Фурье по ним
                            // снова и снова: полосы стояли задранными, а в режиме
                            // прослушивания не возвращалось приближение — оно
                            // ведёт от того же уровня, и кнопка уезжала за край
                            // экрана (жалоба владельца 13.08).
                            // Кормим кольцо тишиной теми же кадрами: спектр гаснет
                            // через тот же сглаживатель, что и у Web Audio на
                            // смолкшем источнике, — падение полос, а не рывок.
...
impl GrowingSource {
    ...
    /// ждать больше нечего: файл дописан, закачка сорвалась ЛИБО ожидание
    /// упёрлось в потолок.
    ///
    /// ⚠️ ПОТОЛОК ОБЯЗАТЕЛЕН, И ВОТ ПОЧЕМУ (16.08, жалоба «треки снова стали
    /// загружаться по 30 секунд»). Без него это ожидание было БЕСКОНЕЧНЫМ, и
    /// висело оно в потоке декодера — то есть до возврата из
    /// `symphonia::probe` поле ошибки оставалось пустым, а `native_status`
    /// честно отвечал `playing: true, position: 0.0`. Со стороны клиента это
    /// неотличимо от «играет молча»: позиция не растёт, и ровно на тридцатой
    /// секунде срабатывает сторож замершего звука
    /// (WATCHDOG_TICK_MS 5с × WATCHDOG_HEAL_TICKS 6 = 30 с) с полной
    /// пере-добычей. Единственным выходом из зависшего чтения был он.
    ///
    /// В 0.3.0 на этот путь пошёл ВЕСЬ холодный SoundCloud HLS (до неё такие
    /// треки качались целиком), поэтому симптом и стал массовым — константы
    /// сторожа при этом не менялись ни на единицу.
    ///
    /// Потолок вдвое меньше первого тика сторожа: отказ обязан случиться
    /// ЗАМЕТНО раньше, чем сторож вообще заподозрит неладное, — тогда клиент
    /// уходит на запасную дорогу сам, а человек ждёт секунды вместо
    /// полуминуты. Больше брать нельзя: разница между «ещё качается» и
    /// «встало» тем дороже, чем дольше молчание.
    fn wait_for(&self, offset: u64) -> bool {
        let deadline = std::time::Instant::now() + GROWING_WAIT_MAX;
        loop {
            ...
            if std::time::Instant::now() >= deadline {
                // Молча: звонящий превратит false в честную ошибку чтения, а
                // та — в failover. Лишний eprintln здесь попал бы в лог на
                // каждый чанк медленной сети.
                return false;
            }
            std::thread::sleep(std::time::Duration::from_millis(20));
        }
