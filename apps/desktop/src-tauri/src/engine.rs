    ...
    let (handle, is_hls) = if let Some(handle) = existing {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            match result {
                ...
                Err(reason) => {
                    // ⚠️ ПРИЧИНА БОЛЬШЕ НЕ ГЛОТАЕТСЯ (13.08). Раньше здесь
                    // стоял `Err(_)` с пометкой «молча: стрим best-effort», и
                    // текст отказа выбрасывался. Цена вскрылась на живом
                    // разборе: в журнале было видно только «стрим не
                    // подтвердился», а был ли это 403 от CDN, обрыв сети или
                    // лимит содержимого — приходилось УГАДЫВАТЬ по косвенным
            ...
            stage0_log(
                ...
                if hls_head_bad {
                    ...
                    "склейка HLS не похожа на fMP4 — стрим отменён, трек играем обычной дорогой"
                        .to_string()
                } else if matches!(confirmed, Ok(false)) {
                    // ⚠️ ЭТА СТРОКА ПОЯВИЛАСЬ ПОТОМУ, ЧТО ПРЕЖНЯЯ ВРАЛА
                    // (13.08, поймано журналом владельца). Ветка отказа одна на
                    // два РАЗНЫХ исхода, а текст был только про таймаут: в
                    // журнале стояло «стрим не завёлся за 8с» там, где весь
                    // путь занял 5.3 с и закачка умерла за 94 мс. Диагностика
                    // уводила ровно в сторону от причины.
                    //
                    // Ok(false) = stream_wait_first_chunk увидел p.failed, то
                    // есть закачка ОТКАЗАЛА, а не запоздала. У YouTube это
                    // почти всегда 403 от googlevideo на уже добытый адрес
                    // (ссылка привязана к IP/сессии; при выкатке PO-токена на
                    // android_vr player-ответ приходит, а байты запрещают).
                    "закачка отказала сразу после старта — обычно это 403 от CDN на добытый адрес; играем запасной дорогой"
                        .to_string()
                } else {
                    format!(
...
/// Первый ответ стрима — он и есть «клик → звук».
///
/// ⚠️ 128 КиБ → 64 КиБ (13.08, по живым замерам владельца). Прежние 128 КиБ
/// брались «с запасом, чтобы декодер точно завёлся», и запас оказался
/// чрезмерным: при 128 кбит/с это ~8 секунд звука, тогда как mp3 заводится с
/// нескольких килобайт, а webm/opus — с заголовков и первого кластера.
///
/// Цена запаса стала видна, когда резолв перестал быть узким местом. Замер
/// 17:43: резолв SoundCloud стабилен (~570 мс на всех треках), а
/// first_chunk_wait скачет 249 → 1713 мс. Разброс всемеро при стабильном
/// резолве — это край CDN: популярный трек лежит в кэше ближайшего узла,
/// редкий тянется с origin. На origin-треках ожидание упирается в СКОРОСТЬ
/// ОТДАЧИ, и половина порога — это половина ожидания.
///
/// ⚠️ КАК ОПРОВЕРГНУТЬ: если данных декодеру перестанет хватать, трек уйдёт
/// запасной дорогой — в журнале появится строка отказа, а в телеметрии
/// path=resolve. Это не тихая деградация: поведение будет не хуже прежнего,
/// зато причина видна. Тогда вернуть 128 КиБ одной строкой.
const STREAM_FIRST_CHUNK: u64 = 64 * 1024;
...
mod warm_tests {
    ...
    fn stream_chunk_end_first_and_next() {
        let total = 4_605_080;
        // Порог первого чанка выражен КОНСТАНТОЙ, а не числом: 13.08 он менялся
        // 128 → 64 КиБ, и тест на литерале ловил бы не смысл, а само значение.
        assert_eq!(stream_chunk_end(0, total), STREAM_FIRST_CHUNK - 1);
        assert_eq!(
            stream_chunk_end(STREAM_FIRST_CHUNK, total),
            STREAM_FIRST_CHUNK + STREAM_NEXT_CHUNK - 1
        );
