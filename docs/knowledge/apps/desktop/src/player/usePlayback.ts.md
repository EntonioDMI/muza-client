...
`usePlayback()` — оркестратор воспроизведения Stage 3: очередь-контекст, реальный движок для каталожных треков (добыча → LRU-кэш → `<audio>` через `AudioEngine`). Локальные файлы играют оттуда же, с диска. Обновлён: 2026-07-16 (кэш источников).
...
**Кэш источников (2026-07-16).** `resolveForTrack` перед сетью смотрит в
`player/sourcesCache.ts` (in-memory Map по track id, TTL 5 мин): повторный
резолв того же трека не дёргает `api.getTrackSources`. Ошибка `resolvePlayable`
инвалидирует запись (rethrow без авто-повтора — семантика ошибок не менялась);
кэшируется сырой ответ, `applySourcePolicy` применяется на каждом резолве;
отказ сервера не кэшируется — оффлайн-ветка (`localHash` → пустая лестница →
`sourcesError` наружу) отрабатывает заново. `VersionsDialog` при смене/сбросе
версии инвалидирует запись рядом с `cacheRemove`. Тесты — в
`usePlayback.test.ts`, describe «кэш источников» (у каждого теста свои track
id: кэш — модульный singleton и переживает соседние тесты). Полный разбор —
корневая база: `usePlayback.ts.md` (раздел «Кэш источников») и
`sourcesCache.ts.md`.
